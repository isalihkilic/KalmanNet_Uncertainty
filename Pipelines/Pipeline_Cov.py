...
class Pipeline_EKF:
    ...
    def setTrainingParams(self, args):
        ...
        self.cov_loss_fn = nn.MSELoss(reduction='mean')
    ...
    def NNTrain(self, SysModel, cv_input, cv_target, cv_target_cov, train_input, train_target, train_target_cov, path_results, \
        ...
        self.MSE_cv_cov_linear_epoch = torch.zeros([self.N_steps])
        self.MSE_cv_cov_dB_epoch = torch.zeros([self.N_steps])

        self.MSE_train_linear_epoch = torch.zeros([self.N_steps])
        self.MSE_train_dB_epoch = torch.zeros([self.N_steps])
        self.MSE_train_cov_linear_epoch = torch.zeros([self.N_steps])
        self.MSE_train_cov_dB_epoch = torch.zeros([self.N_steps])
        ...
        for ti in range(0, self.N_steps):
            ...
            train_target_cov_batch = torch.zeros([self.N_B, SysModel.m, SysModel.m, SysModel.T]).to(self.device)
            x_out_training_batch = torch.zeros([self.N_B, SysModel.m, SysModel.T]).to(self.device)
            x_out_cov_training_batch = torch.zeros([self.N_B, SysModel.m, SysModel.m, SysModel.T]).to(self.device)
            if self.args.randomLength:
                MSE_train_linear_LOSS = torch.zeros([self.N_B])
                MSE_train_cov_LOSS = torch.zeros([self.N_B])
                MSE_cv_linear_LOSS = torch.zeros([self.N_CV])
                MSE_cv_cov_LOSS = torch.zeros([self.N_CV])
            ...
            for index in n_e:
                if self.args.randomLength:
                    ...
                    train_target_cov_batch[ii,:,train_lengthMask[index,:]] = train_target_cov[index,:,train_lengthMask[index,:]]
                else:
                    y_training_batch[ii,:,:] = train_input[index]
                    train_target_batch[ii,:,:] = train_target[index]
                    train_target_cov_batch[ii,:,:,:] = train_target_cov[index]
                ii += 1
            ...
            for t in range(0, SysModel.T):
                x, x_cov = self.model(torch.unsqueeze(y_training_batch[:, :, t],2))
                x_out_training_batch[:, :, t] = torch.squeeze(x)
                x_out_cov_training_batch[:, :, :, t] = torch.reshape(x_cov, x_out_cov_training_batch[:,:,:,t].shape)
                
            
            # Compute Training Loss
            MSE_trainbatch_linear_LOSS = 0
            MSE_traincovbatch_linear_LOSS = 0
            if (self.args.CompositionLoss):
                ...
                #MASKONSTATE=TRUE ÇALIŞMAYABİLİR
                if(MaskOnState):### FIXME: composition loss, y_hat may have different mask with x
                    if self.args.randomLength:
                        ...
                        for index in n_e:# mask out the padded part when computing loss
                            ...
                            MSE_train_cov_LOSS[jj] = self.cov_loss_fn(x_out_cov_training_batch[jj,:,:,train_lengthMask[index]], train_target_cov_batch[jj,:,:,train_lengthMask[index]])
                            jj += 1
                        MSE_trainbatch_linear_LOSS = torch.mean(MSE_train_linear_LOSS)
                        MSE_traincovbatch_linear_LOSS = torch.mean(MSE_train_cov_LOSS)
                    else:                     
                        ...
                        MSE_traincovbatch_linear_LOSS = self.loss_fn(x_out_cov_training_batch, train_target_cov_batch)
                ...
                if(MaskOnState):
                    if self.args.randomLength:
                        ...
                        MSE_traincovbatch_linear_LOSS = self.cov_loss_fn(x_out_cov_training_batch, train_target_cov_batch)
                else: # no mask on state
                    if self.args.randomLength:
                        jj = 0
                        for index in n_e:# mask out the padded part when computing loss
            ...
            self.MSE_train_cov_linear_epoch[ti] = MSE_traincovbatch_linear_LOSS.item()
            self.MSE_train_dB_epoch[ti] = 10 * torch.log10(self.MSE_train_linear_epoch[ti])
            self.MSE_train_cov_dB_epoch[ti] = 10 * torch.log10(self.MSE_train_cov_linear_epoch[ti])
            ...
            # parameters
            full_loss = self.args.mean_weight * MSE_trainbatch_linear_LOSS + MSE_traincovbatch_linear_LOSS * self.args.cov_weight
            full_loss.backward()
            ...
            with torch.no_grad():
                ...
                x_out_cov_cv_batch = torch.empty([self.N_CV, SysModel.m, SysModel.m, SysModel.T_test]).to(self.device)
                # Init Sequence
                if(randomInit):
                    if(cv_init==None):
                        self.model.InitSequence(\
                ...
                for t in range(0, SysModel.T_test):
                    x, x_cov = self.model(torch.unsqueeze(cv_input[:, :, t],2))
                    x_out_cv_batch[:, :, t] = torch.squeeze(x)
                    x_out_cov_cv_batch[:, :, :, t] = torch.reshape(x_cov, x_out_cov_cv_batch[:, :, :, t].shape)
                
                # Compute CV Loss
                MSE_cvbatch_linear_LOSS = 0
                MSE_cvcovbatch_linear_LOSS = 0
                if(MaskOnState):
                    if self.args.randomLength:
                        for index in range(self.N_CV):
                            ...
                            MSE_cv_cov_LOSS[index] = self.cov_loss_fn(x_out_cov_cv_batch[index,:,:,cv_lengthMask[index]], cv_target_cov[index,:,:,train_lengthMask[index]])
                        MSE_cvbatch_linear_LOSS = torch.mean(MSE_cv_linear_LOSS)
                        MSE_cvcovbatch_linear_LOSS = torch.mean(MSE_cv_cov_LOSS)
                    else:          
                        MSE_cvbatch_linear_LOSS = self.loss_fn(x_out_cv_batch[:,mask,:], cv_target[:,mask,:])
                        MSE_cvcovbatch_linear_LOSS = self.cov_loss_fn(x_out_cov_cv_batch[:,:,:,cv_lengthMask[index]], cv_target_cov[:,:,:,train_lengthMask[index]])
                else:
                    if self.args.randomLength:
                        for index in range(self.N_CV):
                        ...
                        MSE_cvcovbatch_linear_LOSS = self.cov_loss_fn(x_out_cov_cv_batch, cv_target_cov)


                # dB Loss
                self.MSE_cv_linear_epoch[ti] = MSE_cvbatch_linear_LOSS.item()
                self.MSE_cv_dB_epoch[ti] = 10 * torch.log10(self.MSE_cv_linear_epoch[ti])
                self.MSE_cv_cov_linear_epoch[ti] = MSE_cvcovbatch_linear_LOSS.item()
                self.MSE_cv_cov_dB_epoch[ti] = 10 * torch.log10(self.MSE_cv_cov_linear_epoch[ti])
                
                if (self.MSE_cv_dB_epoch[ti] < self.MSE_cv_dB_opt):
                    self.MSE_cv_dB_opt = self.MSE_cv_dB_epoch[ti]
                    self.MSE_cv_idx_opt = ti
                    
                    torch.save(self.model, path_results + 'best-model-cov.pt')

            ########################
            ### Training Summary ###
            ########################
            print(ti, "MSE Training :", self.MSE_train_dB_epoch[ti], "[dB]", "MSE Validation :", self.MSE_cv_dB_epoch[ti],
                  "[dB]", "MSE Train Cov: ", self.MSE_train_cov_linear_epoch[ti], "MSE Validation Cov: ", self.MSE_cv_cov_linear_epoch[ti])
        ...
        return [self.MSE_cv_linear_epoch, self.MSE_cv_dB_epoch, self.MSE_cv_cov_linear_epoch, self.MSE_cv_cov_dB_epoch,
                self.MSE_train_linear_epoch, self.MSE_train_dB_epoch, self.MSE_train_cov_linear_epoch, self.MSE_train_cov_dB_epoch]

    def NNTest(self, SysModel, test_input, test_target, test_target_cov, path_results, MaskOnState=False,\
     randomInit=False,test_init=None,load_model=False,load_model_path=None,\
        ...
        if load_model:
            ...
            self.model = torch.load(path_results+'best-model-cov.pt', weights_only=False, map_location=self.device) 

        self.N_T = test_input.shape[0]
        SysModel.T_test = test_input.size()[-1]
        self.MSE_test_linear_arr = torch.zeros([self.N_T])
        self.MSE_test_cov_linear_arr = torch.zeros([self.N_T])
        x_out_test = torch.zeros([self.N_T, SysModel.m, SysModel.T_test]).to(self.device)
        x_out_cov_test = torch.zeros([self.N_T, SysModel.m, SysModel.m, SysModel.T_test]).to(self.device)
        ...
        for t in range(0, SysModel.T_test):
            x, x_cov = self.model(torch.unsqueeze(test_input[:,:, t],2))
            x_out_test[:, :, t] = torch.squeeze(x)
            x_out_cov_test[:, :, :, t] = torch.squeeze(x_cov).reshape((-1, 3, 3))
        ...
        for j in range(self.N_T):# cannot use batch due to different length and std computation  
            if(MaskOnState):
                if self.args.randomLength:
                    ...
                    self.MSE_test_cov_linear_arr[j] = cov_loss_fn(x_out_cov_test[j,:,:,test_lengthMask[j]], test_target_cov[j,:,:,test_lengthMask[j]]).item()
                else:
                    self.MSE_test_linear_arr[j] = loss_fn(x_out_test[j,mask,:], test_target[j,mask,:]).item()
                    self.MSE_test_cov_linear_arr[j] = cov_loss_fn(x_out_cov_test[j,:,:,:], test_target_cov[j,:,:,:]).item()
            else:
                if self.args.randomLength:
        ...
        self.MSE_test_cov_linear_avg = torch.mean(self.MSE_test_cov_linear_arr)
        self.MSE_test_cov_dB_avg = 10 * torch.log10(self.MSE_test_cov_linear_avg)

        self.MSE_symmetricity = cov_loss_fn(x_out_cov_test, x_out_cov_test.permute(0, 2, 1, 3))
        self.MSE_symmetricity_dB = 10 * torch.log10(self.MSE_symmetricity)
        ...
        str = self.modelName + "-" + "MSE Cov Test:"
        print(str, self.MSE_test_cov_dB_avg, "[dB]")
        str = self.modelName + "-" + "Covariance Symmetricity [dB] (0 is optimal):"
        print(str, self.MSE_symmetricity_dB)
        # Print Run Time
