...
def DataGenWithStateUncertainty(args, SysModel_data, fileName):
    assert not hasattr(SysModel_data, "Sigma_posterior")
    ##################################
    ### Generate Training Sequence ###
    ##################################
    SysModel_data.GenerateBatch(args, args.N_E, args.T, randomInit=args.randomInit_train)
    train_input = SysModel_data.Input
    train_targets = [SysModel_data.Target, SysModel_data.Sigma_posterior]
    ### init conditions ###
    train_init = SysModel_data.m1x_0_batch #size: N_E x m x 1
    ### length mask ###
    if args.randomLength:
    ...
    SysModel_data.GenerateBatch(args, args.N_CV, args.T, randomInit=args.randomInit_cv)
    cv_input = SysModel_data.Input
    cv_targets = [SysModel_data.Target, SysModel_data.Sigma_posterior]
    cv_init = SysModel_data.m1x_0_batch #size: N_CV x m x 1
    ### length mask ###
    if args.randomLength:
        cv_lengthMask = SysModel_data.lengthMask
    ...
    SysModel_data.GenerateBatch(args, args.N_T, args.T_test, randomInit=args.randomInit_test)
    test_input = SysModel_data.Input
    test_targets = [SysModel_data.Target, SysModel_data.Sigma_posterior]
    test_init = SysModel_data.m1x_0_batch #size: N_T x m x 1
    ### length mask ###
    if args.randomLength:
        test_lengthMask = SysModel_data.lengthMask
    ...
    #################
    if(args.randomLength):
        torch.save([train_input, train_targets, cv_input, cv_targets, test_input, test_targets,train_init, cv_init, test_init, train_lengthMask,cv_lengthMask,test_lengthMask], fileName)
    else:
        torch.save([train_input, train_targets, cv_input, cv_targets, test_input, test_targets,train_init, cv_init, test_init], fileName)
